...
func (s *Server) handleFollowRequest() http.HandlerFunc {
	type Req struct {
		...
		Option   string `json:"option" validate:"lowercase|contains:accept,reject"`
	}
	return func(w http.ResponseWriter, r *http.Request) {
		var request Req
		sourceID, ok := r.Context().Value(ctxUserID).(string)
		...
		if request.Option == "reject" {
			...
			// nF := model.NewFollow()
			// nF.SourceID = sourceID
			// nF.TargetID = req.TargetID

			// if err := s.store.Follow().Create(*nF); err != nil {
			// 	s.error(w, http.StatusUnprocessableEntity, err)
			// 	return
			// }
