<script lang="ts">
	...
	import { Globe, LockClosed } from 'svelte-radix';
			...
			<div class="h-8 relative mx-0 sm:mx-4">
				...
				>
					...
					{#if $currentUserStore.id !== user.data.id}
						...
						{:else}
							<button
								class="text-sm px-5 rounded-md {user.data.privacy === 'private'
									? 'bg-muted'
									: 'bg-primary'}"
								on:click={async () => {
									// if (user.data.privacy === 'private') {
									// 	toast.error("user is private you can't follow");
									// 	return;
									// }

									const res = await follow(user.data.id);
									console.log(res);
									const followingResp = await getUserFollowing($currentUserStore.id);
									if (!followingResp.ok) {
										toast.error("Couldn't get following " + followingResp.message);
					...
					<div class="flex items-center ml-2 border px-[4px] rounded-md border-primary/50">
						{#if user.data.privacy === 'public'}
							<Globe class="h-5" />
						{:else}
							<LockClosed class="h-5" />
						{/if}
