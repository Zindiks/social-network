<script lang="ts">
	...
	onMount(async () => {
		...
		console.log(
			'usernotif',
			userNotifResp.notifications.map((notification) => {
				if (notification.type_id == 1) {
					notification.view_buttons = false;
				} else {
					notification.view_buttons = true;
				}
				return notification;
			})
		);
			...
			{#if notifications != undefined && notifications.length != 0}
				{#each [...notifications].reverse() as status (status.id)}
					<li transition:fly={{ y: 200, duration: 500 }} class=" mx-auto max-w-96">
						<div
							class="flex items-center p-2 border hover:bg-neutral-100 hover:dark:bg-neutral-800 dark:border-neutral-800 rounded w-full my-1"
						>
							<span class="m-1">👋</span>
							...
							<div class="w-full relative">
								<div class="flex items-center">
									...
									<p class="text-xs ml-2">{status.message}</p>
								</div>
								{#if status.view_buttons}
									<button
										class="border hover:bg-neutral-100 hover:dark:bg-neutral-800 dark:border-neutral-800 rounded cursor-pointer"
										on:click={async () => {
											if (status.type_id == 3) {
												const resp = await resolveNotifGroup(status, 'accept');
												if (resp.ok)
													notificationStore.update((old) =>
														old.filter((notif) => notif.id != status.id)
													);
											} else if (status.type_id == 2) {
												const resp = await resolveNotifFollow(status, 'accept');
												if (resp.ok)
													notificationStore.update((old) =>
														old.filter((notif) => notif.id != status.id)
													);
									...
									<button
										...
										on:click={async () => {
											if (status.type_id == 3) {
												const resp = await resolveNotifGroup(status, 'reject');
												if (resp.ok)
													notificationStore.update((old) =>
														old.filter((notif) => notif.id != status.id)
													);
											} else if (status.type_id == 2) {
												const resp = await resolveNotifFollow(status, 'reject');
												if (resp.ok)
													notificationStore.update((old) =>
														old.filter((notif) => notif.id != status.id)
													);
							...
							>
								<Cross1 class="text-neutral-500 w-3  cursor-pointer" />
							</button>
						</div>
					</li>
				{/each}
