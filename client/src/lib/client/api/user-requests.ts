...
export async function follow(target_id: string) {
  ...
  try {
    const resp = await fetch(`${PUBLIC_LOCAL_PATH}/api/v1/auth/follow/${target_id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Request-Method": "GET",
...
export async function updatePrivacy(privacy: string) {

  try {
    await fetch(`${PUBLIC_LOCAL_PATH}/api/v1/auth/user/privacy/${privacy}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Request-Method": "GET",
