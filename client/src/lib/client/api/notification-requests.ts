...
type notifGroup = ReturnType<string>
export async function resolveNotifGroup(notification: CustomNotification, option: "accept" | "reject", customFetch: Fetch = fetch): Promise<notifGroup> {
  try {
    const body = {
      target_id: notification.source_id,
      option,
      group_id: notification.parent_id
    }

    const fetchResp = await customFetch(`${PUBLIC_LOCAL_PATH}/api/v1/auth/group/request`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Request-Method": "POST",
...
export async function resolveNotifFollow(notification: CustomNotification, option: "accept" | "reject", customFetch: Fetch = fetch): Promise<notifGroup> {
  try {
    console.log(notification.source_id)
    const body = {
      target_id: notification.source_id,
      option
    }

    const fetchResp = await customFetch(`${PUBLIC_LOCAL_PATH}/api/v1/auth/follow/request`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Request-Method": "POST",
